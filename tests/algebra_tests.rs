...
use cim_subject::{
    ...
    SubjectAlgebra, AlgebraOperation, CompositionRule, 
    Result, SubjectError,
};
use std::sync::Arc;
...
fn test_sequential_composition() {
    ...
    let result = algebra.compose(&validate, &process, AlgebraOperation::Sequence).unwrap();
    ...
    let left_assoc = algebra.compose(
        &algebra.compose(&validate, &process, AlgebraOperation::Sequence).unwrap(),
        &ship,
        AlgebraOperation::Sequence
    ).unwrap();
    
    let right_assoc = algebra.compose(
        &validate,
        &algebra.compose(&process, &ship, AlgebraOperation::Sequence).unwrap(),
        AlgebraOperation::Sequence
    ).unwrap();
...
fn test_parallel_composition() {
    ...
    // Compose in parallel
    let email_sms = algebra.compose(&email, &sms, AlgebraOperation::Parallel).unwrap();
    let all_notifications = algebra.compose(&email_sms, &push, AlgebraOperation::Parallel).unwrap();
    ...
    let reverse = algebra.compose(&sms, &email, AlgebraOperation::Parallel).unwrap();
...
fn test_choice_composition() {
    ...
    // Create choices
    let traditional = algebra.compose(
        &credit,
        &debit,
        AlgebraOperation::Choice { condition: "card_type".to_string() }
    ).unwrap();
    
    let all_payment = algebra.compose(
        &traditional,
        &crypto,
        AlgebraOperation::Choice { condition: "payment_method".to_string() }
    ).unwrap();
...
fn test_subject_transformations() {
    ...
    let anon = algebra.compose(
        &user,
        &user,
        AlgebraOperation::Transform { name: "anonymize".to_string() }
    ).unwrap();
    ...
    let upgraded = algebra.compose(
        &user,
        &user,
        AlgebraOperation::Transform { name: "upgrade_version".to_string() }
    ).unwrap();
...
fn test_context_injection() {
    ...
    let public = algebra.compose(
        &internal,
        &internal,
        AlgebraOperation::Inject { context: "public".to_string() }
    ).unwrap();
    ...
    let partner = algebra.compose(
        &internal,
        &internal,
        AlgebraOperation::Inject { context: "partner_api".to_string() }
    ).unwrap();
...
fn test_complex_compositions() {
    ...
    let order_inventory = algebra.compose(
        &order,
        &inventory,
        AlgebraOperation::Sequence
    ).unwrap();
    ...
    let payment_shipping = algebra.compose(
        &payment,
        &shipping,
    ...
    let complete_flow = algebra.compose(
        &order_inventory,
        &payment_shipping,
        AlgebraOperation::Sequence
    ).unwrap();
...
fn test_algebraic_properties() {
    ...
    let par_then_seq = algebra.compose(
...
fn test_algebra_error_handling() {
    ...
    let result = algebra.compose(
        &subject,
        &subject,
        AlgebraOperation::Transform { name: "nonexistent".to_string() }
    );
    ...
    let result = algebra.compose(
        &user_subject,
        &user_subject,
        AlgebraOperation::Transform { name: "restricted".to_string() }
    );
