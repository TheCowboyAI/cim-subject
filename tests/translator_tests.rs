...
use cim_subject::{
    Subject, SubjectParts, Pattern, 
    translator::{Translator, TranslationRule, TranslatorBuilder},
    Result,
};
use std::sync::Arc;
...
fn test_wildcard_translation() {
    ...
    let subjects = vec![
        ...
        ("legacy.inventory.depleted.v3", "modern.inventory.depleted.v2"),
    ];
...
fn test_legacy_system_migration() {
    let translator = TranslatorBuilder::new()
        .custom("monolith_migration", TranslationRule::new(
    ...
    let migrations = vec![
        ("monolith.service.order_created.v1", "orders.order.created.v2"),
        ("monolith.service.inventory_updated.v1", "inventory.stock.updated.v2"),
        ("monolith.service.user_registered.v1", "users.profile.registered.v2"),
    ];
