...
use cim_subject::{
    ...
    SubjectAlgebra, 
    translator::{TranslationRule, Translator},
    permissions::{PermissionsBuilder, Operation, Policy}
};
use std::sync::Arc;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (i, loan) in test_loans.iter().enumerate() {
        println!("\n  Loan {}: ${:.0} @ {:.0}% LTV, FICO {}, Doc Type: {:?}", 
            i + 1, loan.loan_amount, (loan.loan_amount / loan.property_value * 100.0), loan.fico_score, loan.doc_type);
    ...
    for lender in &lenders {
        println!("    {} ({:?}) → {}", lender.name, lender.tier, lender.response_pattern.as_str());
    }
    ...
    for (i, quote) in sorted_quotes.iter().take(3).enumerate() {
        ...
        println!("  Lock: {} days, Expires: {}", quote.lock_days, quote.expires_at.format("%Y-%m-%d %H:%M:%S"));
    }
    ...
    let shopping_strategy = vec![
        ("Tier 1: Prime Lenders", Pattern::new("lending.lenders.prime.>")?, 5),
        ("Tier 2: Alt-A Lenders", Pattern::new("lending.lenders.alta.>")?, 10),
        ("Tier 3: Non-QM Lenders", Pattern::new("lending.lenders.nonqm.>")?, 15),
    ];
...
fn shop_rates(loan: &LoanProfile, lenders: &Vec<Lender>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("    LTV: {:.0}%", (loan.loan_amount / loan.property_value * 100.0));
...
fn simulate_rate_responses(
    ...
    for lender in qualify_lenders(lenders, loan) {
        ...
        let fico_adjustment = if loan.fico_score >= 760 { -0.25 }
            else if loan.fico_score >= 720 { 0.0 }
            else if loan.fico_score >= 680 { 0.25 }
            else { 0.5 };
        ...
        quotes.push(RateQuote {
            ...
            rate,
            apr: rate + 0.125,
            points: if lender.tier == LenderTier::Prime { 0.0 } else { 1.0 },
...
fn initiate_rate_lock(
    ...
    println!("  Rate: {:.3}%", quote.rate);
    println!("  Points: {} (${:.0})", quote.points, loan.loan_amount * quote.points / 100.0);
    println!("  Lock Period: {} days", quote.lock_days);
    println!("  Total Fees: ${:.0}", quote.fees + (loan.loan_amount * quote.points / 100.0));
    ...
    let lock_permissions = PermissionsBuilder::new()
        ...
        .allow("lending.locks.>", &[Operation::Publish, Operation::Subscribe])?
...
fn create_response_translator() -> Result<Translator, Box<dyn std::error::Error>> {
    ...
    translator.register_rule("prime_normalize", TranslationRule::new(
        "prime_normalize",
    ...
    translator.register_rule("nonqm_normalize", TranslationRule::new(
        "nonqm_normalize",
    ...
    translator.register_rule("alta_normalize", TranslationRule::new(
        "alta_normalize",
