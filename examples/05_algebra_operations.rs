...
//! 
//! This example demonstrates algebraic operations for composing and transforming subjects.

use cim_subject::{
    Subject, SubjectAlgebra, AlgebraOperation, CompositionRule,
    Pattern, algebra::{Transformation, TransformFn}
};
use std::sync::Arc;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  {} → {} = {}", validate.as_str(), process.as_str(), sequence.as_str());
    ...
    println!("  {} ⊗ {} = {}", send_email.as_str(), send_sms.as_str(), parallel.as_str());
    ...
    let choice = algebra.compose(
        &credit_payment, 
        &debit_payment, 
        AlgebraOperation::Choice { condition: "payment_method == 'credit'".to_string() }
    )?;
    println!("  {} ⊕ {} (when {}) = {}", 
    ...
    let v2_subject = algebra.compose(
        ...
        AlgebraOperation::Transform { name: "upgrade_v1_to_v2".to_string() }
    )?;
    
    println!("  Transform {} → {}", v1_subject.as_str(), v2_subject.as_str());
    ...
    println!("  Project {} with [context, event_type] = {}", 
        full_event.as_str(), 
        projected.as_str()
    );
    ...
    let injected = algebra.compose(
        ...
        AlgebraOperation::Inject { context: "tenant-123".to_string() }
    )?;
    
    println!("  Inject context 'tenant-123' into {} = {}", 
        base_subject.as_str(), 
        injected.as_str()
    );
    ...
    let complete_workflow = algebra.compose(
        &payment_phase,
        &order_confirm,
        AlgebraOperation::Sequence
    )?;
    ...
    println!("  Steps:");
    println!("    1. ({} ⊗ {})", order_validate.as_str(), inventory_check.as_str());
