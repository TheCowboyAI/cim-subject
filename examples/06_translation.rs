...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    version_translator.register_rule(
        "v1_to_v2",
        TranslationRule::new(
            "v1_to_v2",
            Pattern::new("*.*.*.v1")?,
    ...
    legacy_translator.register_rule(
        "legacy_to_modern",
        TranslationRule::new(
            "legacy_to_modern",
            Pattern::new("*_*_*")?,
    ...
    context_translator.register_rule(
        "add_tenant",
        TranslationRule::new(
            "add_tenant",
    ...
    // Test pipeline
    let raw_subjects = vec!["ORDERS.Commands.Order.CREATE", "inventory.events.stock"];
    ...
    validating_translator.register_rule(
    ...
    for event_str in events {
        match Subject::new(event_str) {
            Ok(event) => match validating_translator.translate(&event) {
