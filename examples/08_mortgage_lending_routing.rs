...
use cim_subject::{
    ...
    AlgebraOperation, SubjectAlgebra
};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let submissions = vec![
        ("BRK-GOLD-001", "lending.gold.001.northeast.submissions.property.submit"),
        ("BRK-GOLD-002", "lending.gold.002.northeast.submissions.property.submit"),
        ("BRK-SILV-001", "lending.silver.001.northeast.submissions.property.submit"),
    ];
    
    // Use patterns to detect multiple submissions
    let multi_submit_pattern = Pattern::new("lending.*.*.*.submissions.property.submit")?;
    
    println!("  Detecting multiple submissions for property {}:", property_id);
    ...
    let regional_rules = HashMap::from([
        ("california", vec!["lending.compliance.california.disclosure.required"]),
    ...
    let avm_routes = HashMap::from([
        (PropertyType::SingleFamily, "lending.valuation.avm.residential"),
        (PropertyType::MultiFamily, "lending.valuation.avm.multifamily"),
        (PropertyType::Commercial, "lending.valuation.avm.commercial"),
        (PropertyType::Construction, "lending.valuation.manual.required"),
    ]);
    ...
    let lender_patterns = vec![
        ("Tier 1 Lenders", Pattern::new("lending.lenders.tier1.>")?, vec![BrokerTier::Platinum]),
        ("Tier 2 Lenders", Pattern::new("lending.lenders.tier2.>")?, vec![BrokerTier::Gold, BrokerTier::Silver]),
        ("Tier 3 Lenders", Pattern::new("lending.lenders.tier3.>")?, vec![BrokerTier::Bronze]),
    ];
...
fn route_application(
    ...
    let broker = brokers.get(&app.broker_id)
        .ok_or("Broker not found")?;
    
    println!("\n  Application {} from {} ({:?})", app.id, broker.name, broker.tier);
