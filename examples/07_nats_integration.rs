...
//! - Add to Cargo.toml: async-nats = "0.33", tokio = { version = "1", features
//!   = ["full"] }
...
impl NatsService {
    ...
    async fn publish_with_identity(
        ...
        headers.insert("X-Correlation-ID".to_string(), vec![identity
            .correlation_id
            .to_string()]);
        headers.insert("X-Causation-ID".to_string(), vec![identity
            .causation_id
            .to_string()]);
        headers.insert("X-Message-ID".to_string(), vec![identity
            .message_id
            .to_string()]);

        // Publish to NATS
        self.client
            .publish_with_headers(subject.as_str().to_string(), headers, payload)
            .await?;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let order_service = NatsService::new("Order Service", nats_url, order_permissions, vec![
    ...
    let mut sub = order_service
        .client
        .subscribe("orders.commands.>".to_string())
        .await?;
        ...
        handle_order_command(&command, command_payload, command_identity.clone()).await?
    {
        ...
        println!(
            "  Event payload: {}",
            String::from_utf8_lossy(&event_payload)
        );
    }
    ...
    impl EventStore {
        ...
        fn replay_for_correlation(
            &self,
            correlation_id: &CorrelationId,
        ) -> Vec<&(Subject, MessageIdentity, Vec<u8>)> {
            self.events
                .iter()
    ...
    for (subject_str, description) in events {
        ...
        event_store.append(subject, event_identity, description.as_bytes().to_vec());
    }

    // Replay events for correlation
    println!(
        "\n  Replaying events for correlation: {}",
        root_identity.correlation_id
    );
    ...
    let query_service =
        NatsService::new("Query Service", nats_url, query_permissions, vec![]).await?;
