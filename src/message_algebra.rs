...
impl CorrelationChain {
    ...
    pub fn add_message(&mut self, message: MessageIdentity) -> Result<()> {
        ...
        if !message.is_root() {
            ...
            self.causation_graph
                .insert(message.message_id.clone(), parent_id.clone());
    ...
    pub fn get_path_to(&self, message_id: &IdType) -> Result<Vec<&MessageIdentity>> {
        ...
        loop {
            let message = self.messages.get(current_id).ok_or_else(|| {
                CorrelationError::InvalidIdentity("Broken chain detected".to_string())
            })?;
...
impl MessageAlgebra {
    ...
    ///
    /// Returns an error if either message is not found in the chain
    pub fn distance(chain: &CorrelationChain, msg1_id: &IdType, msg2_id: &IdType) -> Result<usize> {
        let path1 = chain.get_path_to(msg1_id)?;
        let path2 = chain.get_path_to(msg2_id)?;
...
mod tests {
    ...
    fn test_add_caused_messages() {
        ...
        assert_eq!(
            chain.get_parent(&child.message_id).unwrap().message_id,
            root.message_id
        );
    ...
    fn test_cycle_detection() {
        ...
        // Note: Creating actual cycles would require bypassing the
        // MessageFactory which enforces proper causation rules
    }
}
